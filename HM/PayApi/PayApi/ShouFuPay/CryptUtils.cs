...
namespace HM.Framework.PayApi.ShouFuPay
{
	public class CryptUtils
	{
		...
		public static string CreateSignWithPrivateKey(string str, X509Certificate2 certEncrypt)
		{
			str = Sha1Sign(str);
			byte[] bytesFromString = getBytesFromString(str, Encoding.UTF8);
			byte[] rgbHash = HashAlgorithm.Create("SHA1").ComputeHash(bytesFromString);
			byte[] b;
			using (RSACryptoServiceProvider rSACryptoServiceProvider = (RSACryptoServiceProvider)certEncrypt.PrivateKey)
			{
				...
				b = rSAPKCS1SignatureFormatter.CreateSignature(rgbHash);
