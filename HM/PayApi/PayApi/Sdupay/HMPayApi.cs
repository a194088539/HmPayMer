...
namespace HM.Framework.PayApi.Sdupay
{
	public class HMPayApi : HMPayApiBase
	{
		...
		private string GetChannelCode(HMChannel channel)
		{
			...
			case HMChannel.ALIPAY_H5_URL:
				return "ALIPAY_H5_URL";
			default:
				return string.Empty;
			}
		}
		...
		protected override HMPayResult PayGatewayBody(HMOrder order)
		{
			...
			string text5 = EncryUtils.MD5(stringBuilder.ToString()).ToLower();
			fail.Code = HMPayState.Success;
			fail.Data = string.Format(postUri + "?app_id={0}&trade_type={1}&total_amount={2}&out_trade_no={3}&notify_url={4}&return_url={5}&client_ip={6}&extra_return_param={7}&sign={8}&interface_version={9}", accountUser, text3, text2, orderNo, notifyUri, returnUri, clientIp, text4, text5, text);
			return fail;
		}
